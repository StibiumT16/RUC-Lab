...
std::pair<IxNodeHandle*, bool> IxIndexHandle::FindLeafPage(const char *key, Operation operation, Transaction *transaction) {
    ...
    if (operation == Operation::FIND) {
        ...
        node->page->WLatch();
        if ((operation == Operation::INSERT && node->GetSize() < node->GetMaxSize() - 1) || (operation == Operation::DELETE && node->GetSize() > 2)) {
            is_root_page_id_latched = false;
            root_latch_.unlock();
        }
    }
...
bool IxIndexHandle::GetValue(const char *key, std::vector<Rid> *result, Transaction *transaction) {
    ...
    auto res = FindLeafPage(key, Operation::FIND, transaction);
    auto leaf = res.first;
    Rid *v;
...
bool IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    auto res =  FindLeafPage(key, Operation::INSERT, transaction);
    IxNodeHandle *leaf = res.first;
    auto root_is_latched_ = res.second;
    int sz = leaf->GetSize(), new_sz = leaf->Insert(key, value);
    if(sz == new_sz){
...
void IxIndexHandle::InsertIntoParent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    if(old_node->IsRootPage()){
        ...
        //buffer_pool_manager_->UnpinPage({new_root->page->GetPageId().fd,HEADER_PAGE_ID}, true);
        root_latch_.unlock();
        ClearTransactionPageSet(transaction);
        return;
    }
    IxNodeHandle *fa = FetchNode(old_node->GetParentPageNo());
    int sz = fa->Insert(key,  {new_node->GetPageNo(), fa->find_child(old_node) + 1});
    if(sz < fa->GetMaxSize()){
        ClearTransactionPageSet(transaction);
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    auto res = FindLeafPage(key, Operation::DELETE, transaction);
    IxNodeHandle *leaf = res.first;
    auto root_is_latched_ = res.second;
    ...
    bool leaf_should_delete = CoalesceOrRedistribute(leaf, root_is_latched_, transaction);
    leaf->page->WUnlatch();
    if(leaf_should_delete) transaction->AddIntoDeletedPageSet(leaf->page);
...
bool IxIndexHandle::CoalesceOrRedistribute(IxNodeHandle *node, bool root_is_latched, Transaction *transaction) {
    ...
    if(node->IsRootPage()){
        bool root_should_delete = AdjustRoot(node, root_is_latched);
        ClearTransactionPageSet(transaction);
    ...
    IxNodeHandle *bro = FetchNode(fa->ValueAt(!id ? 1 : id - 1));
    bro->page->WLatch();
    if(bro->GetSize() + node->GetSize() >= node->GetMaxSize()){
        Redistribute(bro, node, fa, id, root_is_latched);
        ClearTransactionPageSet(transaction);
        //buffer_pool_manager_->UnpinPage(fa->GetPageId(), true);
        bro->page->WUnlatch();
    ...
    if(Coalesce(&bro, &node, &fa, id, transaction, root_is_latched)) transaction->AddIntoDeletedPageSet(fa->page);
    //buffer_pool_manager_->UnpinPage(fa->GetPageId(), true);
    bro->page->WUnlatch();
    //buffer_pool_manager_->UnpinPage(bro->GetPageId(), true);
    return true;
 ...
 */
bool IxIndexHandle::AdjustRoot(IxNodeHandle *old_root_node, bool root_is_latched) {
    // Todo:
    ...
    if(!old_root_node->IsLeafPage() && old_root_node->GetSize() == 1){
        ...
        //buffer_pool_manager_->UnpinPage({old_root_node->page->GetPageId().fd,HEADER_PAGE_ID}, true);
 ...
 * 注意更新parent结点的相关kv对
 */
void IxIndexHandle::Redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index, bool root_is_latched) {
    // Todo:
    ...
    if(!index){
        ...
        node->insert_pairs(0, neighbor_node->get_key(pos), neighbor_node->get_rid(pos), 1);
...
bool IxIndexHandle::Coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    return CoalesceOrRedistribute(*parent, root_is_latched, transaction);
}
