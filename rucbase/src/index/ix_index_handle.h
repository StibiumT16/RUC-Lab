...
class IxIndexHandle {
   ...
   public:
    ...
    bool AdjustRoot(IxNodeHandle *old_root_node, bool is_root_page_id_latched);

    void Redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index, bool is_root_page_id_latched);
   ...
   private:
    ...
    void ClearTransactionPageSet(Transaction *transaction){
        if (transaction == nullptr) return;
        for (Page *page : *transaction->GetPageSet()) page->WUnlatch();
        transaction->GetPageSet()->clear();
    }
