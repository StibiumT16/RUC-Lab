...
int IxNodeHandle::lower_bound(const char *target) const {
    ...
    while(l < r){
        mid = (l + r) >> 1;
        ix_compare(target, get_key(mid), file_hdr->col_type, file_hdr->col_len) <= 0 ? r = mid : l = mid + 1; 
    }
    return l;
}
...
bool IxNodeHandle::LeafLookup(const char *key, Rid **value) {
    ...
    if(pos !=  page_hdr->num_key && !ix_compare(key, get_key(pos), file_hdr->col_type, file_hdr->col_len)){
        *value = get_rid(pos);
        return 1;
    }
    return 0;
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    memmove(key_pos + n * len, key_pos, (GetSize() - pos) * len);
    memcpy(key_pos, key, n * len);
    memmove(rid_pos + n, rid_pos, (GetSize() - pos) * sizeof(Rid));
...
int IxNodeHandle::Insert(const char *key, const Rid &value) {
    ...
    // 4. 返回完成插入操作之后的键值对数量
    int pos = lower_bound(key);
    if(ix_compare(key, get_key(pos), file_hdr->col_type, file_hdr->col_len) || pos == GetSize()) insert_pair(pos, key, value);
    return GetSize();
}
...
void IxNodeHandle::erase_pair(int pos) {
    ...
    char* key_pos = get_key(pos);
    Rid* rid_pos = get_rid(pos);
    memmove(key_pos, key_pos + file_hdr->col_len, (GetSize() - pos - 1) * file_hdr->col_len);
    memmove(rid_pos, rid_pos + 1, (GetSize() - pos - 1) * sizeof(Rid));
    --page_hdr->num_key;
}
...
int IxNodeHandle::Remove(const char *key) {
    ...
    // 3. 返回完成删除操作后的键值对数量
    int pos = lower_bound(key);
    if(!ix_compare(key, get_key(pos), file_hdr->col_type, file_hdr->col_len) && pos != GetSize()) erase_pair(pos);
    return GetSize();
}
