...
Page *BufferPoolManager::FetchPage(PageId page_id) {
    ...
    if(page_table_.find(page_id) != page_table_.end()){
        Page * page = &pages_[frame_id = page_table_[page_id]];
        ++page->pin_count_;
    ...
    page_table_[page->id_ = page_id] = frame_id;
    ++page->pin_count_;
    replacer_->Pin(frame_id);
...
bool BufferPoolManager::UnpinPage(PageId page_id, bool is_dirty) {
    ...
    if(!--page->pin_count_) replacer_->Unpin(frame_id);
    return true;
}
...
Page *BufferPoolManager::NewPage(PageId *page_id) {
    ...
    page->ResetMemory();
    page_table_[page->id_ = {page_id->fd, page_id->page_no}] = frame_id;
    ++page->pin_count_;
    replacer_->Pin(frame_id);
    return page;
}
