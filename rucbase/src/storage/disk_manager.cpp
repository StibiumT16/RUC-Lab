...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    // 2.调用write()函数
    // 注意处理异常
    std::string path = GetFileName(fd);
    if(!is_file(path.c_str())) throw FileNotFoundError(path);
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    if(!is_file(path.c_str())) throw FileNotFoundError(path);
    lseek(fd, page_no * PAGE_SIZE, SEEK_SET);
    if(read(fd, offset, num_bytes) != num_bytes) throw UnixError();
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if(path2fd_.count(path)) throw UnixError();
...
int DiskManager::open_file(const std::string &path) {
    ...
    if(fd < 0) throw UnixError();
    fd2path_[path2fd_[path] = fd] = path;
    return fd;
}
...
void DiskManager::close_file(int fd) {
    ...
    if (fd2path_.find(fd) == fd2path_.end()) throw UnixError();
    std::string path = GetFileName(fd);
    if(!is_file(path.c_str())) throw FileNotFoundError(path);
    int res = close(fd);
    if(res < 0) throw UnixError();
    path2fd_.erase(path);
    fd2path_.erase(fd);
}
