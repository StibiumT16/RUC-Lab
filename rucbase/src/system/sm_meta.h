...
struct TabMeta {
    ...
    bool is_col(const std::string &col_name) const {
        // lab3 task1 Todo
        for(auto x : cols) if(x.name == col_name) return true;
        return false;
        // lab3 task1 Todo End
    }
    /**
    ...
    std::vector<ColMeta>::iterator get_col(const std::string &col_name) {
        // lab3 task1 Todo
        auto it = std::find_if(cols.begin(), cols.end(), [&](const ColMeta &col) {return col.name == col_name;});
        if (it == cols.end()) { throw ColumnNotFoundError(col_name); }
        return it;
        // lab3 task1 Todo End
    }
