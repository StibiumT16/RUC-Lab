...
void SmManager::close_db() {
    ...
    db_.name_.clear();
    db_.tabs_.clear();
    for (auto &x : fhs_)  rm_manager_->close_file(x.second.get());
	fhs_.clear();
    for (auto &x : ihs_)  ix_manager_->close_index(x.second.get());
	ihs_.clear();
    if (chdir("..") < 0)  throw UnixError();
    // lab3 task1 Todo End
}
...
void SmManager::drop_table(const std::string &tab_name, Context *context) {
    ...
    for(auto &x : tab.cols)
        if(x.index) drop_index(tab_name, x.name, context);
