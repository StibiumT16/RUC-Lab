...
// keywords
%token ORDER BY
%token SHOW TABLES CREATE TABLE DROP DESC INSERT INTO VALUES DELETE FROM ASC LIMIT
...
%type <sv_order> orderStmt;
%type <sv_order_collist> order_colList;
%type <sv_int> orderType;
%%
start:
        stmt ';'
    {
...
dml:
    ...
    |   SELECT selector FROM tableList optWhereClause orderStmt
    {
        $$ = std::make_shared<SelectStmt>($2, $4, $5, $6.first.first, $6.first.second, $6.second);
    }
    ;
...
orderStmt:
    /*empty*/ 
    {
        $$ = std::make_pair(std::make_pair(std::vector<std::shared_ptr<Col>>{}, std::vector<int>{}), -1);
    }
    |   ORDER BY order_colList
    {
        $$ = std::make_pair(std::make_pair($3.first, $3.second), -1);
    }
    |   ORDER BY order_colList LIMIT VALUE_INT
    {
        $$ = std::make_pair(std::make_pair($3.first, $3.second), $5);
    }
    ;
...
order_colList:
    col orderType
    {
        $$ = std::make_pair(std::vector<std::shared_ptr<Col>>{$1}, std::vector<int>{$2});
    }
    |   order_colList ',' col orderType
    {
        $$.first.push_back($3);
        $$.second.push_back($4);
    }
