...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid &rid, Context *context) const {
    ...
    // auto lm_ = context->lock_mgr_;
    // context->txn_->SetState(TransactionState::GROWING);
    // lm_->LockSharedOnRecord(context->txn_, rid, fd_);
    auto record = std::make_unique<RmRecord>(file_hdr_.record_size);
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    char *slot = page_handle.get_slot(rid.slot_no);
    memcpy(record->data, slot, file_hdr_.record_size);
    record->size = file_hdr_.record_size;
    // context->txn_->SetState(TransactionState::SHRINKING);
    // lm_->Unlock(context->txn_, LockDataId(fd_, rid, LockDataType::RECORD));
    return record;
}
...
Rid RmFileHandle::insert_record(char *buf, Context *context) {
    ...
    // auto lm_ = context->lock_mgr_;
    // context->txn_->SetState(TransactionState::GROWING);
    // lm_->LockExclusiveOnRecord(context->txn_, rid, fd_);
    ...
    context->txn_->GetWriteSet()->emplace_back(new WriteRecord(WType::INSERT_TUPLE, disk_manager_->GetFileName(fd_), rid, RmRecord(file_hdr_.record_size, slot)));
    // context->txn_->SetState(TransactionState::SHRINKING);
    // lm_->Unlock(context->txn_, LockDataId(fd_, rid, LockDataType::RECORD));
    return rid;
}

Rid RmFileHandle::new_insert_record(char *buf, Context *context) {
    // Todo:
    // 1. 获取当前未满的page handle
    // 2. 在page handle中找到空闲slot位置
    // 3. 将buf复制到空闲slot位置
...
void RmFileHandle::delete_record(const Rid &rid, Context *context) {
    ...
    page_handle.page->WUnlatch();
    buffer_pool_manager_->UnpinPage(page_handle.page->GetPageId(), true);
    context->txn_->GetWriteSet()->emplace_back(new WriteRecord(WType::DELETE_TUPLE, disk_manager_->GetFileName(fd_), rid, RmRecord(file_hdr_.record_size, page_handle.get_slot(rid.slot_no))));
    // context->txn_->SetState(TransactionState::SHRINKING);
    // lm_->Unlock(context->txn_, LockDataId(fd_, rid, LockDataType::RECORD));
}


void RmFileHandle::new_delete_record(const Rid &rid, Context *context) {
    // Todo:
    // 1. 获取指定记录所在的page handle
    // 2. 更新page_handle.page_hdr中的数据结构
    // 注意考虑删除一条记录后页面未满的情况，需要调用release_page_handle()
...
void RmFileHandle::update_record(const Rid &rid, char *buf, Context *context) {
    ...
    page_handle.page->WLatch();
    char *slot = page_handle.get_slot(rid.slot_no);
    char *res = new char[2 * file_hdr_.record_size];
    memcpy(res, slot, file_hdr_.record_size);
    memcpy(res + file_hdr_.record_size, buf, file_hdr_.record_size);
    memcpy(slot, buf, file_hdr_.record_size);
    page_handle.page->WUnlatch();
    buffer_pool_manager_->UnpinPage(page_handle.page->GetPageId(), true);
    context->txn_->GetWriteSet()->emplace_back(new WriteRecord(WType::UPDATE_TUPLE, disk_manager_->GetFileName(fd_), rid, RmRecord(file_hdr_.record_size * 2, res)));
    // context->txn_->SetState(TransactionState::SHRINKING);
    // lm_->Unlock(context->txn_, LockDataId(fd_, rid, LockDataType::RECORD));
}

void RmFileHandle::new_update_record(const Rid &rid, char *buf, Context *context) {
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    // auto lm_ = context->lock_mgr_;
    // context->txn_->SetState(TransactionState::GROWING);
    // lm_->LockExclusiveOnRecord(context->txn_, rid, fd_);
...
RmPageHandle RmFileHandle::create_page_handle() {
    ...
    return ~file_hdr_.first_free_page_no ? fetch_page_handle(file_hdr_.first_free_page_no) : create_new_page_handle();
}
