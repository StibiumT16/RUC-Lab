...
bool LRUReplacer::Victim(frame_id_t *frame_id) {
    ...
    frame_id_t tmp = LRUlist_.front();
    LRUlist_.pop_front();
    if(LRUhash_.find(tmp) == LRUhash_.end()) return false;
    LRUhash_.erase(*frame_id = tmp);
    return true;
}
...
void LRUReplacer::Unpin(frame_id_t frame_id) {
    ...
    //  选择一个frame取消固定
    std::scoped_lock lock{latch_};
    if(LRUhash_.size() >= max_size_)  return;
    if(LRUhash_.find(frame_id) != LRUhash_.end()) return;
    LRUlist_.push_back(frame_id);
    LRUhash_[frame_id]= std::prev(LRUlist_.end(), 1);
}
