    ...
    void interp_sql(const std::shared_ptr<ast::TreeNode> &root, txn_id_t *txn_id, Context *context) {
        if (auto x = std::dynamic_pointer_cast<ast::Help>(root)) {
            ...
            *(context->offset_) = strlen(help_info);
            ...
            // select;
            std::vector<Condition> conds = interp_where_clause(x->conds);
            std::vector<TabCol> sel_cols, order_cols;
            for (auto &sv_sel_col : x->cols) {
                TabCol sel_col = {.tab_name = sv_sel_col->tab_name, .col_name = sv_sel_col->col_name};
                sel_cols.push_back(sel_col);
            }
            for (auto &sv_order_col : x->order_cols) {
                TabCol sel_col = {.tab_name = sv_order_col->tab_name, .col_name = sv_order_col->col_name};
                order_cols.push_back(sel_col);
            }
            SetTransaction(txn_id, context);
            ql_manager_->select_from(sel_cols, x->tabs, conds, order_cols, x->orders, x->top, context);
            if(context->txn_->GetTxnMode() == false)
                txn_mgr_->Commit(context->txn_, context->log_mgr_);
