    ...
    void beginTuple() {
        ...
        for (auto &cond : fed_conds_) {
            if (cond.is_rhs_val && cond.op != OP_NE && cond.lhs_col.col_name == index_col.name) {
                ...
                // lab3 task2 todo
                else if (cond.op == OP_LT) upper = ih->lower_bound(rhs_key);
                else if (cond.op == OP_GT) lower = ih->upper_bound(rhs_key);
                else if (cond.op == OP_LE) upper = ih->upper_bound(rhs_key);
                else if (cond.op == OP_GE) lower = ih->lower_bound(rhs_key);
