...
GLOBAL_CUT(int _n, const set<int>& _V, const set<int>& _SSV, vector<int>* &_Adj) : n(_n), V(_V), SSV(_SSV), Adj(_Adj){
	pru.resize(n + 1);
	deposit.resize(n + 1);
	dist.resize(n + 1);
}
...
void sweep(int u){
	...
	for(auto v : Adj[u]){
		if(pru[v]) continue;
		++deposit[v];
		if(SSV.find(u) != SSV.end() || deposit[v] >= k) sweep(v);
	}
}
...
vector<set<int> > kvcc(int n, vector<int>* E, bool need_cal_ssv, set<int>& ssv){
	vector<set<int> > ans;
	...
	for(int i = 1; i <= n; ++i){
		...
		if(cut.empty()){
			...
			for(auto x : sV){
				...
				vector<set<int> > res = kvcc(n, pAdj, false, pssv);
				for(auto v : res){
					set<int> tmp;
...
int main(){
	...
	set<int>ssv;
	vector<set<int> >  ans = KVCC::kvcc(n, E, true, ssv);
